...
class AdobePdf(Dependency):
    ...
    def _run_once(self):
        ...
        self.a.execute(
            f'"c:\\Program Files (x86)\\Adobe\\Reader {v}.0\\Reader\\AcroRd32.exe"',
            cucksync=True,
        )
    ...
    def run(self):
        if self.version.startswith("11") and self.filename.endswith(".msp"):
            log.debug(
                "We have a MSI upgrade package, we need the vanilla AdbeRdr installer."
            )
            ...
            self.a.execute("C:\\%s -nos_oC:\\AdobeFiles -nos_ne" % self.filename)
            ...
            try:
                exit_code = self.a.execute(
                    ...
                    "ALLUSERS=1 EULA_ACCEPT=YES SUPPRESS_APP_LAUNCH=YES" % self.filename
                ).get("exit_code")
        ...
        self.a.execute(
            ...
            "/v bCheckForUpdatesAtStartup /t REG_DWORD /d 0 /f" % key_version
        )
        ...
        self.a.execute(
            'reg add "HKEY_LOCAL_MACHINE\\SOFTWARE\\'
            'Policies\\Adobe\\Acrobat Reader\\%s.0\\FeatureLockDown" '
            "/v bEnhancedSecurityStandalone /t REG_DWORD /d 0 /f" % key_version
        )
        self.a.execute(
            'reg add "HKEY_LOCAL_MACHINE\\SOFTWARE\\'
            'Policies\\Adobe\\Acrobat Reader\\%s.0\\FeatureLockDown" '
            "/v bEnhancedSecurityInBrowser /t REG_DWORD /d 0 /f" % key_version
        )
