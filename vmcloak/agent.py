...
class Agent(object):
    ...
    def hostname(self, hostname):
        ...
        cmdline = (
            'wmic computersystem where name="%(oldname)s" '
    ...
    def static_ip(self, ipaddr, netmask, gateway, interface):
        ...
        command = ('netsh interface ip set address name="%s" static %s %s %s 1') % (
            interface,
            ipaddr,
            netmask,
            gateway,
        ...
        self.ipaddr = ipaddr
        log.debug(f"Waiting for agent to be reachable on: {self.ipaddr}:{self.port}")
        wait_for_agent(self)

    def dns_server(self, ipaddr):
        """Set the IP address of the DNS server."""
        command = (
            'netsh interface ip set dns name="Local Area Connection" static %s' % ipaddr
        )
    ...
    def click(self, window_title, button_name):
        ...
        self.execute('C:\\vmcloak\\click.exe "%s" "%s"' % (window_title, button_name))
    ...
    def click_async(self, window_title, button_name):
        ...
        self.execute(
            'C:\\vmcloak\\click.exe "%s" "%s"' % (window_title, button_name),
            cucksync=True,
        )
    ...
    def resolution(self, width, height):
        ...
        self.execute(
            "C:\\Python3\\python.exe C:\\vmcloak\\resolution.py %s %s" % (width, height)
        )
